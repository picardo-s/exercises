...
building::building()            //конструктор
{
    ...
    for (int j = 0; j < NUM_FLOORS; j++)    //для каждого этажа
    {
        ...
        _itoa_s(j + 1, ustring, 10);          //на экран
        cout << setw(3) << ustring;
...
void building::record_floor_reqs()
{
    ...
    cout << "Press Enter to call the elevator: ";
    ...
    cout << "Where are you going to be lifted? (u/d): ";
...
void elevator::decide()         //принятие решения
{
    ...
    {
        if (ptrBuilding->get_cars_dir(k) == UP)
            count2++;
    }
    ...
    for(int i = 0; i < NUM_CARS; i++)
        ...
        {
            if (ptrBuilding->get_cars_floor(i) == n && ptrBuilding->get_floor_req(DN, n))
                flag = false;
        }

    if (count2 < count1 && flag && (current_dir == UP || current_dir == STOP)
        && requests_above && car_between_up)
        {
            current_dir = UP;
            return;
        }
                                  ...
                                  //пунктов назначения
{
    ...
    set_cursor_pos(1, 22);
    cout << "Elevator " << (car_number + 1)
        << " stopped on the floor № " << (current_floor + 1)
        << "\nDestination floor (0 to end input)";
